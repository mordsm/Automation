...
async function activateRequest(script, cmd_idx) {
  var pos = cmd_idx;
  for (var i = pos; i < script.commands.length; i++) {
    ...
    await eval(script.commands[cmd_idx - 1]);
  }
}
...
async function get_user_answer(question) {
  try {
    ...
    //elem.click();
    if (question.toLowerCase().includes("user code"))
        {   answer = GM_getValue("code");
            return answer;}
    else
        if (question.toLowerCase().includes("password"))
            { answer = GM_getValue("password");
              return answer;}
